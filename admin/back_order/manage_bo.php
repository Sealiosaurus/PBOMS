<?php 
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT p.*,s.name as supplier FROM purchase_order_list p inner join seller s on p.supplier_id = s.id  where p.id = '{$_GET['id']}'");
    if($qry->num_rows >0){
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <?php 
                            $supplier = $conn->query("SELECT * FROM `seller` where status = 1 order by `name` asc");
