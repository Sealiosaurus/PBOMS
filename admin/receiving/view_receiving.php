...
if($qry->num_rows >0){
    ...
    if($from_order == 1){
        $po_qry = $conn->query("SELECT p.*,s.name as seller FROM `purchase_order_list` p inner join `seller` s on p.seller_id = s.id where p.id= '{$form_id}' ");
        ...
        $qry = $conn->query("SELECT b.*,s.name as seller,p.po_code FROM back_order_list b inner join seller s on b.seller_id = s.id inner join purchase_order_list p on b.po_id = p.id  where b.id = '{$form_id}'");
            if($qry->num_rows >0){
                foreach($qry->fetch_array() as $k => $v){
                    if($k == 'id')
                    $k = 'bo_id';
...
<div class="card card-outline card-danger">
    <div class="card-header">
        <h1>Received Order Details - <?php echo $po_code ?></h1>
    </div>
    ...
    <div class="card-footer py-1 text-center">
        ...
        <a class="btn btn-flat btn-danger" href="<?php echo base_url.'/admin?page=receiving/manage_receiving&id='.(isset($id) ? $id : '') ?>">Edit</a>
