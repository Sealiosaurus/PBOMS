...
if(isset($_GET['id'])){
    ...
    if($qry->num_rows >0){
        ...
        if($from_order == 1){
            $qry = $conn->query("SELECT p.*,s.name as seller FROM purchase_order_list p inner join seller s on p.seller_id = s.id  where p.id = '{$form_id}'");
            if($qry->num_rows >0){
                foreach($qry->fetch_array() as $k => $v){
                    if($k == 'id')
                    $k = 'po_id';
            ...
            $qry = $conn->query("SELECT b.*,s.name as seller FROM back_order_list b inner join seller s on b.seller_id = s.id  where b.id = '{$form_id}'");
            if($qry->num_rows >0){
                foreach($qry->fetch_array() as $k => $v){
                    if($k == 'id')
                    $k = 'bo_id';
...
if(isset($_GET['po_id'])){
    $qry = $conn->query("SELECT p.*,s.name as seller FROM purchase_order_list p inner join seller s on p.seller_id = s.id  where p.id = '{$_GET['po_id']}'");
    if($qry->num_rows >0){
        foreach($qry->fetch_array() as $k => $v){
            if($k == 'id')
            $k = 'po_id';
...
if(isset($_GET['bo_id'])){
    $qry = $conn->query("SELECT b.*,s.name as seller,p.po_code FROM back_order_list b inner join seller s on b.seller_id = s.id inner join purchase_order_list p on b.po_id = p.id  where b.id = '{$_GET['bo_id']}'");
    if($qry->num_rows >0){
        foreach($qry->fetch_array() as $k => $v){
            if($k == 'id')
            $k = 'bo_id';
...
<div class="card card-outline card-danger">
    <div class="card-header">
        <h1><?php echo !isset($id) ? "Receive Order from ".$po_code : 'Update Received Order' ?></h1>
    </div>
    <div class="card-body">
            ...
            <div class="container-fluid">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select id="seller_id" name="seller_id" class="custom-select select2">
                            <option <?php echo !isset($seller_id) ? 'selected' : '' ?> disabled></option>
                            <?php 
                            $seller = $conn->query("SELECT * FROM `seller` where status = 1 order by `name` asc");
                            while($row=$seller->fetch_assoc()):
                            ?>
                            <option value="<?php echo $row['id'] ?>" <?php echo isset($seller_id) && $seller_id == $row['id'] ? "selected" : "" ?> ><?php echo $row['name'] ?></option>
                            <?php endwhile; ?>
                            </select>
                        </div>
                    </div>
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <tbody>
                        ...
                        if(isset($po_id)):
                        if(!isset($bo_id))
                        $qry = $conn->query("SELECT p.*,i.name,i.description FROM `po_items` p inner join product i on p.item_id = i.id where p.po_id = '{$po_id}'");
                        else
                        $qry = $conn->query("SELECT b.*,i.name,i.description FROM `bo_items` b inner join product i on b.item_id = i.id where b.bo_id = '{$bo_id}'");
    ...
    <div class="card-footer py-1 text-center">
        <button class="btn btn-flat btn-danger" type="submit" form="receive-form">Receive</button>
