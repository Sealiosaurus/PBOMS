...
Class Master extends DBConnection {
	...
	function save_supplier(){
		...
		$check = $this->conn->query("SELECT * FROM `seller` where `name` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Seller name already exists.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id)){
				$res['msg'] = " Seller successfully added!";
				$id = $this->conn->insert_id;
			}else{
				$res['msg'] = " Seller has been successfully updated!";
			}
	...
	function delete_supplier(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `seller` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Seller has been successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_item(){
		...
		$check = $this->conn->query("SELECT * FROM `product` where `name` = '{$name}' and `seller_id` = '{$seller_id}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Product already exists under selected seller.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success'," Product successfully added!");
			else
				$this->settings->set_flashdata('success'," Product has been successfully updated!");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_item(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Product has been successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_po(){
		...
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success'," Order successfully created!");
			}else{
				$this->settings->set_flashdata('success'," Order details have been successfully updated!");
			}
		}
	...
	function delete_po(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Order has been successfully deleted.");
			if($bo->num_rows > 0){
	...
	function save_receiving(){
		...
		$data = "";
		foreach($_POST as $k =>$v){
			if(!in_array($k,array('id','bo_code','seller_id','po_id')) && !is_array($_POST[$k])){
				if(!is_numeric($v))
				$v= $this->conn->real_escape_string($v);
				if(!empty($data)) $data .=", ";
				$data .=" `{$k}` = '{$v}' ";
		...
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success'," Order has been successfully received.");
			}else{
				$this->settings->set_flashdata('success'," Received order has been successfully updated.");
			}
		}
	...
	function delete_receiving(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Received order has been successfully deleted.");
	...
	function delete_bo(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Back order details have been successfully deleted.");
	...
	function save_return(){
		...
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success'," New returned item record has been successfully created!");
			}else{
				$this->settings->set_flashdata('success'," Returned item record has been successfully updated!");
			}
		}
	...
	function delete_return(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Returned item record has been successfully deleted.");
			if(isset($res)){
				$this->conn->query("DELETE FROM `stock_list` where id in ({$res['stock_ids']})");
			}
		}else{
